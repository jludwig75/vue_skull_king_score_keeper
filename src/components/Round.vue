<template>
  <v-card v-if="round != null">
    <v-card-title>
      ...
      <v-btn color="primary" v-if="round.state == 0 && round.all_bids_placed()" @click="round.start()">Start
        Round</v-btn>
      <span v-if="round.state == 1 && round.all_tricks_won_set()">
        <v-btn color="primary" v-if="round.total_tricks_won() <= round.round_number" @click="round.end()">End
          Round</v-btn>
